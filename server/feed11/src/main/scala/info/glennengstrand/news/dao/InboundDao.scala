...
class InboundDao extends DataAccess[Inbound] {
  ...
  override def fetchSingle(id: Int): Future[Inbound] = {
    Future.successful(Inbound(None, None, None, None, None))
  }
  ...
  override def fetchMulti(id: Int): Future[Seq[Inbound]] = {
    ...
    } yield Inbound(Option(toLink(r.getInt(1).toLong)), Option(toLink(id)), Option(format(r.getInstant(0))), Option(r.getString(2)), Option(r.getString(3)))
